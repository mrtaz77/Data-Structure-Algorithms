...
long long maxFlow(vector<pair<int,long long>> adj[],int n,int s,int t){
...
int main(){
    ...
    for(int k=1;k<=t;k++){
        int m,n;
        cin>>m>>n;
        vector<int> height(m+n,0);
        vector<int> age(m+n,0);
        vector<int> isDivorced(m+n,0);
        for(int i=0;i<m+n;i++){
            cin>>height[i]>>age[i]>>isDivorced[i];
        }
        vector<pair<int,long long int>> *adj = new vector<pair<int,long long int>>[m+n+2];

        for(int i=0;i<m;i++){
            addEdge(adj,0,i+1,1);
        }
        for(int i=0;i<n;i++){
            addEdge(adj,m+i+1,m+n+1,1);            
        }
        for(int i=0;i<m;i++){
            for(int j=m;j<m+n;j++){
                if(abs(height[i] - height[j]) <= 12 && abs(age[i]-age[j]) <= 5 && !(isDivorced[i] ^ isDivorced[j])){
                    addEdge(adj,i+1,j+1,1);
                }
            }
        }
        //print(adj,m+n+2);
        cout<<"Case "<<k<<": "<<maxFlow(adj,m+n+2,0,m+n+1)<<endl;
        delete []adj;
    }
}