...
long double bitmaskDp(int current,long int mask,Graph &G,vector<vector<int>>& dp){
...
long double Exact_TSP(Graph& G){
    vector<vector<int>> dp((1 << G.getV()), vector<int>(G.getV(), -1));
    // cout<<"Exact_TSP : "<<endl;
    auto tour = bitmaskDp(0,1,G,dp);
    return tour;
}
...
long double Metric_Approximation_TSP(Graph& G){
    ...
    // cout<<endl;
    return Calculate_Tour_Length(G,dfsTree);
    // return 0;
}
